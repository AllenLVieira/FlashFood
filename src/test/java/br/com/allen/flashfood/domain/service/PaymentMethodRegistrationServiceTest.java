...
class PaymentMethodRegistrationServiceTest {
  ...
  @Autowired private PaymentMethodRegistrationService underTest;
  ...
  void shouldThrowsEmptyResultDataAccessExceptionWhenSavePayment() {
    ...
    paymentMethod.setId(123L);
    assertThrows(
        EmptyResultDataAccessException.class, () -> underTest.savePaymentMethod(paymentMethod));
    verify(paymentMehodRepository).save(any());
  }
  ...
  void shouldThrowsEntityInUseExceptionWhenDeletePaymentMethod() {
    doThrow(new DataIntegrityViolationException("An error occured"))
        .when(paymentMehodRepository)
        .deleteById(any());
  ...
  void shouldThrowPaymentMethodNotFoundExceptionWhenPaymentMethodIsEmpty() {
    ...
    assertThrows(
        PaymentMethodNotFoundException.class, () -> underTest.findPaymentMethodOrElseThrow(123L));
    verify(paymentMehodRepository).findById(any());
  }
  ...
  void shouldThrowPaymentMethodNotFoundExceptionWhenPaymentMethodDoesNotExists() {
    ...
    assertThrows(
        EmptyResultDataAccessException.class, () -> underTest.findPaymentMethodOrElseThrow(123L));
    verify(paymentMehodRepository).findById(any());
  }
}
