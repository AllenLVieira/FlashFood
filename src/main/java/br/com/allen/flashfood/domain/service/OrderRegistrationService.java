...
public class OrderRegistrationService {
  ...
  private void validateItems(DeliveryOrder order) {
    order
        ...
        .forEach(
            item -> {
              Product product =
                  productService.findProductOrElseThrow(
  ...
  private void validateOrder(DeliveryOrder order) {
    ...
    Restaurant restaurant =
        restaurantService.findRestaurantOrElseThrow(order.getRestaurant().getId());
    PaymentMethod paymentMethod =
        paymentMethodService.findPaymentMethodOrElseThrow(order.getPaymentMethod().getId());
    ...
    if (restaurant.doesNotAcceptPaymentMethod(paymentMethod)) {
      throw new BusinessException(
          String.format(
              "Payment method '%s' is not accepted by this restaurant.",
